...
def process_generation(
    ...
    results_dict = {}
    panel_number = 0
    outdir = f"output/out_{datetime.datetime.now().strftime('%Y%m%d-%H%M%S')}"
    os.makedirs(outdir)
    ...
    for real_prompts_ind in real_prompts_inds:
        ...
        panel_number += 1
        results_dict[real_prompts_ind].save(f'{outdir}/img{panel_number}.png')
        yield [results_dict[ind] for ind in results_dict.keys()]
    ...
    if _comic_type != "No typesetting (default)":
        ...
        my_comic=get_comic(total_results, _comic_type, captions=captions, font=font)
        for img in my_comic:
            panel_number += 1
            img.save(f'{outdir}/img{panel_number}.png')
        total_results = (
            my_comic + total_results
        )
